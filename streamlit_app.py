...
def extract_final_result(output):
    ...
    for line in lines:
        ...
        if any(
            skip in line
            for skip in ["╭", "│", "╰", "🚀", "📋", "└──", "Assigned to:", "Status:"]
        ):
            if current_block:
...
def main():
    st.set_page_config(
        page_title="LocalBreeze KNMI documentation Assistant",
        page_icon="🔬",
        layout="wide",
    )

    st.title("🔬 LocalBreeze 🤖")
    st.markdown("Enter a topic to in KNMI's documentation.")
    ...
    if st.button("🚀 Start Research", type="primary", disabled=not topic.strip()):
        if topic.strip():
            with st.spinner(
                f"🔍 Researching '{topic}'... This may take a few minutes.",
            ):
                result, error = run_research(topic.strip())
                print(f"Result from LocalBreeze : {result}")
    ...
    if "research_result" in st.session_state:
        ...
        st.divider()
        st.subheader(
            f"Research Results: {st.session_state.get('research_topic', 'Unknown Topic')}",
        )
        ...
        with col1:
            ...
            summary = result_text.split('"list_of_references"')[0].split('"summary":')[
                1
            ][
                1:-2
            ]  # Strip " and ,
        ...
        with col2:
            ...
            references = result_text.split('"list_of_references":')[1]
            with st.container():
                st.markdown(references.split())

            # Add download buttons for refs
            for r in references.split():
                st.download_button(
                    label="📥 Download ref as Text",
                    data=r,
                    file_name=f"ref_{r}.txt",
                    mime="text/plain",
                )
